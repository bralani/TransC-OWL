...
vector<vector<double> > entity_vec, relation_vec, class_mu, class_covariance;
vector<double> concept_r;
vector<pair<int, int> > ins_right, ins_wrong, sub_right, sub_wrong;
vector<pair<int, int> > subclassOf_triples;
...
// Calcola la distanza di Mahalanobis tra due punti x e y utilizzando la matrice di covarianza diagonale
double calculateMahalanobisDistance(const vector<double>& x, const vector<double>& y, const vector<double>& cov_t) {
    size_t dim = x.size();
    ...
    double distance = 0.0;
    for (size_t i = 0; i < dim; ++i) {
        distance += (diff[i] * diff[i]) / cov_t[i];
    }
...
bool checkSubClass(int concept1, int concept2){
    ...
    // Calcola la distanza tra i due vettori
    double distance = 0.0;
    for (size_t i = 0; i < dim; i++) {
        distance += pow(class_mu[concept1][i] - class_mu[concept2][i], 2);
    }
    distance = sqrt(distance);

    // Calcola le dimensioni relative degli ellissoidi
    double log_det_h = 0.0;
    double log_det_t = 0.0;
    for (size_t i = 0; i < dim; i++) {
        log_det_h += log(class_covariance[concept1][i]);
    }
    for (size_t i = 0; i < dim; i++) {
        log_det_t += log(class_covariance[concept2][i]);
    }
    double volumes_h = 0.5 * dim * log(2 * M_PI) + 0.5 * log_det_h;
    double volumes_t = 0.5 * dim * log(2 * M_PI) + 0.5 * log_det_t;

    double size_ratio = volumes_h - volumes_t;

    double prob = -(distance + size_ratio);

    if(prob > threshold_subclassOf) {
        return true;
    } else {
        return false;
    }
...
Result minimize_scalar(double (*func)(double, const std::vector<double>&, const std::vector<double>&, double), const std::vector<double>& lambdas, const std::vector<double>& v_squared, double tau)
{
    double a = 0.0;
    double b = 0.5;
    double c = 1.0;

    double fa = func(a, lambdas, v_squared, tau);
    double fb = func(b, lambdas, v_squared, tau);
    double fc = func(c, lambdas, v_squared, tau);

    while (c - a > 1e-6)
    {
        double d = (a + b) / 2.0;
        double fd = func(d, lambdas, v_squared, tau);

        if ((fa >= 0.0 && fd >= 0.0) || (fa < 0.0 && fd < 0.0))
        {
            a = d;
            fa = fd;
        }
        ...
        b = (a + c) / 2.0;
        fb = func(b, lambdas, v_squared, tau);
    }

    Result result;
    result.is_positive = (fb >= 0.0);
    result.fun = fb;

    return result;
}

double K_function(double s, const std::vector<double>& lambdas, const std::vector<double>& v_squared, double tau)
{
    double sum = 0.0;
    for (size_t i = 0; i < lambdas.size(); i++)
    {
        sum += v_squared[i] * ((s * (1.0 - s)) / (1.0 + s * (lambdas[i] - 1.0)));
    }
    return 1.0 - (1.0 / (tau * tau)) * sum;
}

bool ellipsoid_intersection_test(const std::vector<double>& Sigma_A, const std::vector<double>& Sigma_B, const std::vector<double>& mu_A, const std::vector<double>& mu_B, double tau)
{
    size_t size = Sigma_A.size();
    std::vector<double> lambdas(size);
    std::vector<double> Phi(size * size, 0.0);
    std::vector<double> mu_diff(size);
    std::vector<double> v_squared(size);

    // Calcolo lambdas
    for (size_t i = 0; i < size; i++)
    {
        lambdas[i] = Sigma_A[i] / Sigma_B[i];
    }
    ...
    for (size_t i = 0; i < size; i++)
    {
        mu_diff[i] = mu_A[i] - mu_B[i];
    }

    // Calcolo v_squared
    for (size_t i = 0; i < size; i++)
    {
        v_squared[i] = dot_product(std::vector<double>(&Phi[i * size], &Phi[i * size + size]), mu_diff);
        v_squared[i] *= v_squared[i];
    }

    Result res = minimize_scalar(K_function, lambdas, v_squared, tau);
    bool result = res.is_positive;
...
bool intersection_classes(int concept1, int concept2){
    return ellipsoid_intersection_test(class_covariance[concept1], class_covariance[concept2], class_mu[concept1], class_mu[concept2], 1.0);
}
...
void init(){
    entity_vec.clear(); class_covariance.clear(); class_mu.clear(); relation_vec.clear();
    concept_r.clear();
    ins_right.clear(); ins_wrong.clear(); sub_right.clear(); sub_wrong.clear();
}

pair<double, double> test_subclass(){
    double TP_sub = 0, TN_sub = 0, FP_sub = 0, FN_sub = 0;

    // cicla subclassOf_triples
    for(int i = 0; i < subclassOf_triples.size(); ++i){
        int concept1 = subclassOf_triples[i].first;
        int concept2 = subclassOf_triples[i].second;

        if(checkSubClass(concept1, concept2)) {
            TP_sub++;
        }else {
            FN_sub++;
        }
        if(!checkSubClass(concept2, concept1)) {
            TN_sub++;
        }else {
            FP_sub++;
        }
...
void runValidSubClass(){
    double sub_best_answer = 0, sub_best_delta = 0;
    
    for(int i = -200; i <= 200; ++i){
        threshold_subclassOf = i;
        pair<double, double> ans = test_subclass();
        if(ans.second > sub_best_answer){
            sub_best_answer = ans.second;
            sub_best_delta = threshold_subclassOf;
        }
    }
    threshold_subclassOf = sub_best_delta;
}
...
void loadEmbedding(string epoch) {
    ...
    FILE* f2;
    FILE* f3;
    f1 = fopen(("../data/" + dataSet + "/Output/transprob/entity2vec_"+epoch+".txt").c_str(), "r");
    f2 = fopen(("../data/" + dataSet + "/Output/transprob/relation2vec_"+epoch+".txt").c_str(), "r");
    f2 = fopen(("../data/" + dataSet + "/Output/transprob/classmu2vec_" +epoch+ ".txt").c_str(), "r");
    f3 = fopen(("../data/" + dataSet + "/Output/transprob/classcov2vec_" +epoch+ ".txt").c_str(), "r");
...
void prepare(){
	...
	getline(subclassof_file, tmp_str);
	while (getline(subclassof_file, tmp_str))
	{
		string::size_type pos = tmp_str.find(' ', 0);
		int classe1 = atoi(tmp_str.substr(0, pos).c_str());
		int classe2 = atoi(tmp_str.substr(pos + 1).c_str());
        pair<int, int> p = pair<int, int>(classe1, classe2);
        subclassOf_triples.push_back(p);
	}
...
/*
    ...
    */
    ...
    FILE* results = fopen(("../data/" + dataSet + "/Output/transprob/results_expl.csv").c_str(), "w");

    double max_lift = 0;

    vector<double> lift_arr;
    for(int epoch = 500; epoch <= 1000; epoch += 400) {
        ...
        // B = istanze non di c2
        // N = istanze totali
        // (supporto(ist. di c1 e non di c2) / N) / (supporto(ist. di c1) / N) * (supporto(ist. non di c2) / N)
