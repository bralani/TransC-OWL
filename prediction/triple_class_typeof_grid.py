...
epoch = 100
            ...
            #file_in = open(files_src + 'transprob/'+str(search[i])+'/'+str(search[j])+'/'+str(search[k])+'/entity2vec_'+str(epoch)+'.txt', 'r')
            ...
            with open("result_typeof_gridsearch.txt", "a") as myfile:
                myfile.write(str(search[idx])+"," +str(search[jdx])+"," +str(search[kdx])+"," +str(np.mean(acc_o)) + ","+str(np.mean(prec_o)) + ","+str(np.mean(rec_o))+","+str(np.mean(fpr_o))+"\n")
            epoch += 50
            kdx += 1
        jdx += 1
    idx += 1
